...
normal run logs ``agent_decision`` records, and that deterministic mode needs
no Ollama server. All tests run fully offline (deterministic provider is the
default). Provider-parser tests live in ``test_provider.py``.
"""
...
from src.agents.provider import DeterministicProvider, make_provider
